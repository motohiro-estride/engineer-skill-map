...
> **Note**: `import.meta.env.*` ではなく `process.env.*` を使う理由は、Astro/Vite の server-side コードと **tsx スクリプト** (`scripts/migrate-excel.ts` 等) の両方で同じ config を共有できるため。`import.meta.env` は Vite/Astro 専用で、`tsx` 直接実行では存在しない。
